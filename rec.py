...
def getRecommendationsSim(prefs,person,similarity=sim_pearson, sim_weight = 1):
    '''
...
def loo_cv(prefs, sim, sim_weight):
    """
    ...
    for person in prefs:
        ...
        for movie in movies:
            ...
            rec = getRecommendationsSim(prefs, person, similarity=sim, sim_weight=sim_weight)
            # if sim == 'pearson':
...
# Create the list of recommendation for person
def getRecommendedItems(prefs,itemMatch,user, sim_weight = 1):
    '''
...
def main():
    ...
    while not done: 
        ...
        #     print()file_dir ='data/ml-100k/'
        #     # path from current directorydatafile ='u.data'
        #     # ratings fileitemfile ='u.item'# movie titles fileprint ('Reading "%s" dictionary from file'% datafile)prefs = from_file_to_dict(path, file_dir+datafile, file_dir+itemfile)print('Number of users: %d\nList of users [0:10]:'% len(prefs), list(prefs.keys())[0:10] 
